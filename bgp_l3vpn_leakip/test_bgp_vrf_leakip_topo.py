...
def test_bgp_vrf_ipleak():
    ...
    for subnet in range(0, 10):
        netkey = '10.101.{}.0/24'.format(subnet)
    ...
    test_func = functools.partial(topotest.router_json_cmp,
        tgen.gears['r1'], 'show ip bgp vrf r1-cust2 ipv4 json', expect)
    _, res = topotest.run_and_expect(test_func, None, count=12, wait=0.5)
    assertmsg = 'expected routes in "show ip bgp vrf r1-cust2 ipv4" output'
    assert res is None, assertmsg
